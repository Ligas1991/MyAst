	...
	for (int i = 0; i < nst.length; i++) {
		...
		if(mas[i][j]!=0 ||  i != j){ // Ёлемент i j всегда должен быть 0 , это путь к самому себе
				...
				if(mas[i][j] == 0 && i<nst.length-1){ //  ищем пустые элементы 
				    if(mas[i+1][j] != 0){ // провер€ем следующую €чейку если 0 но увеличиваем j
					mas[i][j] = mas[i+1][j] + mas[i][j-1];// наш пустой элемент равен  элемент из следующей €чейки + элемент данной €чейки равный следующей €чейки 
				    }
				}
